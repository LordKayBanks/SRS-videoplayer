...
const rangeBasic = [2.5, 3, 3.5, 3, 3.5, 3, 3.5] /*3.0*/
// const rangeFast = [
//   2.5, 3, 3.25, 3.75, 3.25, 3.75, 3.5, 4, 3.5, 4, 3.5, 4,
// ]; /*3.5*/
const rangeFast = [2.5, 3.5, 4, 3.5, 4, 3.5, 4]
...
function notifyReplayStatus() {
  ...
  let videoStat =
    reviews &&
    reviews[video.origin?.path]?.replayHistory[`split-${currentSplit}`]?.count
  ...
  },
  {
    ...
    action(e) {
      const volume = Math.min(
        ...
        Math.max(
          0,
          Math.round(v.volume * 100 + (e.code === 'KeyW' ? 5 : -5)) / 100
        )
      )
