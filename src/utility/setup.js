...
export function setupForStandardTrackingMode(reviewModeSate) {
  ...
  replayConfig.startOffset = convertToNearestX(
    getVideoCurrentTime(),
    replayConfig.interval
  )
}
...
export const seekToTime = function (value) {
  ...
  if (seekToTime < 0) {
    setVideoPosition(0)
  } else if (seekToTime > getVideoTotalDuration())
    setVideoPosition(getVideoTotalDuration())

  setVideoPosition(seekToTime)
  notify.display(
    `Current Position: <${toMinutesSeconds(
      getVideoCurrentTime()
    )}> of <${toMinutesSeconds(getVideoTotalDuration())}>`
  )
}
...
function notifyReplayStatus() {
  ...
  let videoStat =
    reviews &&
    reviews[getVideoPath()]?.replayHistory[`split-${currentSplit}`]?.count
